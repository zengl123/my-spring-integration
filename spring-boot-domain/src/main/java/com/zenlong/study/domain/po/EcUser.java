...
public class EcUser extends BaseModel implements Serializable {
    ...
    @ApiModelProperty(value = "邮箱地址", name = "email", example = "13763645521@163.com", required = true)
    private String email;
    @NotBlank(message = "电话号码不能为空")
    @Pattern(regexp = REGEX_MOBILE, message = "电话号码不正确")
    @ApiModelProperty(value = "电话号码", name = "phone", example = "13763645521", required = true)
    private String phone;
    @NotBlank(message = "密保问题不能为空")
    @Size(min = 2, max = 30, message = "问题字数限制在30字以内")
    @ApiModelProperty(value = "密保问题", name = "question", example = "问题一", required = true)
    private String question;
    @NotBlank(message = "密保答案不能为空")
    @Size(min = 2, max = 30, message = "答案字数限制在30字以内")
    @ApiModelProperty(value = "密保答案", name = "answer", example = "答案一", required = true)
