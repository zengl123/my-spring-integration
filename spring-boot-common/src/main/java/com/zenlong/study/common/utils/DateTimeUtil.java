...
public class DateTimeUtil {
     ...
     */
    public static final FastDateFormat DATE_FORMAT_CHINA_FORMAT_DATETIME_PATTERN = FastDateFormat.getInstance(CHINA_FORMAT_DATETIME_PATTERN);
    ...
    public static long between(String fTime, String oTime) {
        if (fTime.length() >= 10 && oTime.length() >= 10) {
            ...
            // 指标监控--日志
            throw new CusException("日期格式有误====----【日期-开始时间 {}】【日期-结束时间 {}】----====【正确格式为:{}】", fTime, oTime, "yyyy-MM-dd || yyyy-MM-dd hh:mm:ss");
        }
    }
